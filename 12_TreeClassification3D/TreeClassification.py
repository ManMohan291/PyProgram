...
def PlotTreeSplit(ax,X,SplitFeature,SplitValue,Level): 
    ...
    ax.plot_surface(U,V,W,alpha=0.5)
...
def PlotTree(ax,X,y,Node):
    if(len(Node.children)!=0):
        ...
        ax.scatter(X[np.where(y==0),0],X[np.where(y==0),1],X[np.where(y==0),2],marker=".",facecolors='r')
        ax.scatter(X[np.where(y==1),0],X[np.where(y==1),1],X[np.where(y==1),2],marker=".",facecolors='g')
        ax.scatter(X[np.where(y==2),0],X[np.where(y==2),1],X[np.where(y==2),2],marker=".",facecolors='b')
    return
